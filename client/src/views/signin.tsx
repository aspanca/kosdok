...
export const SigninPage = ({ initialMode: propMode }: SigninPageProps = {}) => {
  ...
  const onLoginSubmit = loginForm.handleSubmit(async (data) => {
    ...
    try {
      ...
      router.push(user.type === "clinic" ? "/clinic-profile" : "/");
    } catch (err) {
            ...
            {mode === "login" ? (
              <form onSubmit={onLoginSubmit} className="space-y-5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link href={{ pathname: "/signin", query: { mode: "login" } }} className="text-[13px] font-[600] tracking-[0.39px] text-primary hover:underline">
              ...
              <form onSubmit={onSignupSubmit} className="space-y-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <label className="text-[13px] font-normal tracking-[0.39px] text-[#757b8c]">
                    ...
                    <Link href="/privacy-policy" className="text-primary hover:underline">{t("auth.signin.terms")}</Link>{" "}
                    {t("auth.signin.and")}{" "}
                    <Link href="/privacy-policy" className="text-primary hover:underline">{t("auth.signin.privacyPolicy")}</Link>
                  </label>
                </div>
