...
export const Results = () => {
  ...
  const search = useMemo<ResultsSearch>(() => {
    ...
    return {
      q: q || undefined,
      city: city || undefined,
      type: typeRaw === "clinic" || typeRaw === "doctor" ? typeRaw : undefined,
      serviceId: serviceIdRaw && !isNaN(Number(serviceIdRaw)) ? Number(serviceIdRaw) : undefined,
    };
  }, [searchParams]);
  ...
  const replaceSearch = (next: ResultsSearch) => {
    const params = new URLSearchParams();
    if (next.q) params.set("q", next.q);
    if (next.city) params.set("city", next.city);
    if (next.type) params.set("type", next.type);
    if (next.serviceId !== undefined) params.set("serviceId", String(next.serviceId));
  ...
  useEffect(() => {
    const handle = setTimeout(() => {
      if ((search.q ?? "") !== searchQuery) {
        replaceSearch({ ...search, q: searchQuery || undefined });
      }
    }, 350);
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2 sm:gap-3">
              <Select
                ...
                onValueChange={(value) =>
                  replaceSearch({ ...search, city: value === "all" ? undefined : value })
                }
              >
